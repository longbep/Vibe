...
async def get_spotify_title(url):
    async with aiohttp.ClientSession(
            headers={'User-Agent': 'python-requests/2.20.0'}) as session:
...
async def get_video_data(url, search, bettersearch, loop):
    ...
    if not search and not bettersearch:
        ...
        return Song(source, url, title, description, views, duration,
...
class Music(object):
    def __init__(self):
        if not has_voice:
            raise RuntimeError(
                "youtube_dl install needed in order to use voice")
    ...
    async def play(self):
        ...
        self.voice.play(source,
                        after=lambda error: self.after_func(
                            self.ctx, self.ffmpeg_opts, self.music, self.
                            after_func, self.on_play_func, self.loop))
...
class Song(object):
    def __init__(self, source, url, title, description, views, duration,
                 thumbnail, channel, channel_url, loop):
        self.source = source
        self.url = url
        self.title = title
